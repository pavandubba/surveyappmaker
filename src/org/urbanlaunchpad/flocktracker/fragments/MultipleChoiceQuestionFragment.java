...
public class MultipleChoiceQuestionFragment extends QuestionFragment {
  ...
  private OnClickListener onClickListener = new OnClickListener() {
    ...
    public void onClick(View v) {
      ...
      getListener().onSelectedAnswer(((AnswerView) v).getAnswer().toString());
    }
  };

  public MultipleChoiceQuestionFragment(QuestionActionListener listener, Question question, QuestionType questionType) {
    super(listener, question, questionType);

    answersContainer = (LinearLayout) getView().findViewById(R.id.answer_layout);
  }
  ...
  public void setupLayout() {
    ...
    // Add listeners for answers
      ...
      answersLayout[i] = (AnswerView) getInflater().inflate(R.layout.question_answer, null);
      ...
      answersContainer.addView(answersLayout[i]);
		...
		if (hasOther) {
      answersLayout[numAnswers - 1] = (AnswerView) getInflater().inflate(R.layout.question_answer, null);
      answersLayout[numAnswers - 1].initialize(getQuestion().getType(), null);
