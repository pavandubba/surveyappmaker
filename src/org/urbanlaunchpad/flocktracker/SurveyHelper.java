...
public class SurveyHelper {
    ...
    public static HashMap<Integer, ArrayList<Integer>> selectedTrackingAnswersMap =
    ...
    public SurveyHelper(String username, String jsonSurvey, Context context) {
        ...
        try {
            ...
            this.jtrackerString = this.jsurv.getJSONObject(Surveyor.TRACKER_TYPE)
                                            .toString();
    ...
    static void checkLocationConfig(final Context context) {
        ...
        if (!gps_enabled && !network_enabled) {
            ...
            dialog.setNegativeButton(context.getString(R.string.cancel_location_settings),
              new DialogInterface.OnClickListener() {
    ...
    public void parseChapters() {
        try {
            jchapterlist = jsurv.getJSONObject(Surveyor.SURVEY_TYPE).getJSONArray(
              "Chapters");
...
//						Toast.LENGTH_SHORT).show();
        ...
        try {
            ...
            if (type.equals(Surveyor.TRACKER_TYPE)) {
                synchronized (Surveyor.trackerSubmissionQueue) {
                    ...
                    Iniconfig.prefs
                      .edit()
                      .putStringSet("trackerSubmissionQueue",
                        (Set<String>) Surveyor.trackerSubmissionQueue.clone())
                      .commit();
                ...
                synchronized (Surveyor.surveySubmissionQueue) {
                    ...
                    Iniconfig.prefs
                      .edit()
                      .putStringSet("surveySubmissionQueue",
                        (Set<String>) Surveyor.surveySubmissionQueue.clone())
                      .commit();
      ...
      ArrayList<Integer> selectedAnswers) {
        try {
            if (chapterPosition >= 0) {
                jsurv.getJSONObject(Surveyor.SURVEY_TYPE).getJSONArray("Chapters")
    ...
    public Integer[] findIDPosition(String iDtoFind) {
        ...
        for (int i = 0; i < jchapterlist.length(); ++i) {
            for (int j = 0; j < chapterQuestionCounts[i]; ++j) {
                try {
                    String questionID = jsurv.getJSONObject(Surveyor.SURVEY_TYPE)
                                             .getJSONArray("Chapters").getJSONObject(i)
    ...
    public JSONObject getCurrentQuestion() throws JSONException {
        return jsurv.getJSONObject(Surveyor.SURVEY_TYPE).getJSONArray("Chapters")
